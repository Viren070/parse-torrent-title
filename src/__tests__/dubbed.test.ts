...
describe('Dubbed Detection Tests', () => {
  test('the english dubbed language correctly', () => {
    const result = parseTorrentTitle(
      'Yo-Kai Watch S01E71 DUBBED 720p HDTV x264-W4F'
    );
  ...
  test('dual-audio correctly', () => {
    const result = parseTorrentTitle(
      '[DB] Gamers! [Dual Audio 10bit 720p][HEVC-x265]'
    );
    expect(result.dubbed).toBe(true);
  });
  ...
  test('dual correctly', () => {
    const result = parseTorrentTitle(
      'Fame (1980) [DVDRip][Dual][Ac3][Eng-Spa]'
    );
    expect(result.dubbed).toBe(true);
  });
  ...
  test('dubbing correctly', () => {
    const result = parseTorrentTitle(
      'Vaiana Skarb oceanu / Moana (2016) [720p] [WEB-DL] [x264] [Dubbing]'
    );
    expect(result.dubbed).toBe(true);
  });
