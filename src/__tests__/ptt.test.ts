...
// Helper to create partial result for comparison
type PartialResult = Partial<ParsedResult>;

describe('Parse Torrent Title', () => {
  const testCases: Array<{ title: string; expected: PartialResult }> = [
    {
    ...
    {
      title:
        'Ghost Busters *1984-2021* [720p] [BDRip] [AC-3] [XviD] [Lektor + Dubbing PL] [DYZIO]',
      expected: {
        title: 'Ghost Busters',
        year: '1984-2021',
        resolution: '720p',
  ...
  testCases.forEach(({ title, expected }) => {
    test(title, () => {
      ...
      Object.keys(expected).forEach((key) => {
        const expectedValue = expected[key as keyof PartialResult];
        const actualValue = result[key as keyof ParsedResult];
...
describe('Parse Torrent Title - Additional Cases', () => {
  const additionalCases: Array<{ title: string; expected: PartialResult }> = [
    {
...
describe('Parse Torrent Title - Edge Cases', () => {
  const edgeCases: Array<{ title: string; expected: PartialResult }> = [
    {
      title:
    ...
    {
      title:
        'www.-tamil-blasters.hair-deiva-thirumagal-2011-tamil-1080p-hq-hdrip-x-264-ddp-5.1-640-kbps-aac-3-gb',
      expected: {
        quality: 'HDRip',
        codec: 'x264',
        audio: ['DDP'],
...
describe('Parse Torrent Title - Anime Tests', () => {
  const animeCases: Array<{ title: string; expected: PartialResult }> = [
    {
    ...
    {
      title:
        '[Salieri] Mushoku Tensei - Jobless Reincarnation - 17.5 (1080p) (HDR) [Dual Audio].mkv',
      expected: {
        container: 'mkv',
        dubbed: true,
        episodes: [17],
