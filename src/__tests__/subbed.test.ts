...
describe('parseTorrentTitle - subbed', () => {
  test('SUBFRENCH', () => {
    const result = parseTorrentTitle('Have I Got News For You S53E02 EXTENDED 720p HDTV x264-QPEL SUBFRENCH');
  ...
  test('ESUB', () => {
    const result = parseTorrentTitle('The.Stranger.2020.S01.1080p.NF.WEB-DL.DDP.5.1.x264.ESUB-DDR');
    expect(result.subbed).toBe(true);
  });

  test('PLSUB', () => {
    const result = parseTorrentTitle('Have I Got News For You S53E02 EXTENDED 720p HDTV x264-QPEL PLSUB');
    expect(result.subbed).toBe(true);
  });
  ...
  test('SLOSUBS', () => {
    const result = parseTorrentTitle('Have I Got News For You S53E02 EXTENDED 720p HDTV x264-QPEL SLOSUBS');
    expect(result.subbed).toBe(true);
  });
